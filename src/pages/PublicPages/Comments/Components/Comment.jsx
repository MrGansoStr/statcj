	...
	return (
		<Box component="div" className="mx-3 mt-2 p-3 border rounded-2">
			...
			<Typography component="div" variant="body1" className="fs-bold">
				{GetGender(InfoComment?.Usuario?.gender)}
				{InfoComment?.Usuario?.name} {InfoComment?.Usuario?.lastName}
			</Typography>
			<Typography component="div" variant="body1" className="p-2">
				{InfoComment?.comment}&nbsp;
				<Typography>
					<ThumbUpOutlinedIcon color="secondary" /> {InfoComment?.numberLikes}{" "}
					&nbsp; <ThumbDownOffAltOutlinedIcon color="secondary" />{" "}
					{InfoComment?.numberDislikes} &nbsp; {hours} &nbsp; {date}
				</Typography>
			</Typography>
			{
				<Box
					component="div"
					className="d-flex alig-items-end justify-content-end p-2"
				>
					<a
						onClick={(event) => ChangeShowAction(event, "answer")}
						type="button"
					...
					{isLogged && IsEditable(idUserLogged, InfoComment?.userId) ? (
						<>
							<a
								onClick={(event) => ChangeShowAction(event, "edit")}
								type="button"
				...
				>
					...
					<DialogContent dividers>
						...
						>
							...
							<Box component="div" className="col-4 mx-3">
								<Button
									onClick={(event) => DeleteComment(event, InfoComment)}
									type="button"
									variant="contained"
									color="error"
								>
