...
function PercentileCalculator() {
  ...
  const calculatePercentil = () => {
    if (ThereErrors(rawData) || ThereErrors(TheNumK, "percentil")) {
      ...
      setcalculatedPercentil(
        CalculateAllPercentil(ProcessInput(rawData), TheNumK, grouped),
      );
    }
  };
  ...
  return (
    <Box component="div">
      <Stack
        component="div"
        spacing={3}
        className="p-3 mt-3 d-flex align-items-center justify-content-center"
      >
        <Typography variant="h5">
          Calcular Percentiles, Quartiles y Deciles
        </Typography>
        <TextField
          type="text"
